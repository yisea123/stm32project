...
typedef struct
{
	...
	PinInst mode0; 	//0: full step; 1: half step;
	PinInst mode1; 	//0: full step; 1: half step;
	PinInst mode2; 	//0: full step; 1: half step;
...
typedef struct
{
	...
	uint32_t channel;
	__IO int32_t* ptrSteps;
	__IO uint32_t* ptrCount;
} Motor;


extern __IO int32_t motorSteps[IDX_MOTOR_MAX];
extern __IO uint32_t motorStepsCount[IDX_MOTOR_MAX];
...
extern const uint8_t MotorPinsDefaults[IDX_MOTOR_MAX][8];
