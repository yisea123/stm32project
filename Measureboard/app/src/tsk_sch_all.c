...
{
	SendTskMsg(SCH_ID, TSK_INIT, SCH_TSK_IDLE, NULL);
}
...
//	GetCaliResult(&caliResultRealTime,measSch.measCaliIndex);
	...
	while (TASK_LOOP_ST)
	{
		...
		if (event.status == osEventMessage)
		{
			...
			if ( mainTskState == TSK_SUBSTEP)
			{
				...
				switch (tskState)
				{
					...
					case SCH_MEASURE:
					{
						...
						SendTskMsg(SCH_MEAS_ID, TSK_INIT, schRangeSelection, SchTskFinish);
					...
					case SCH_AR_MEASURE:
					{
						UpdateSchFlushCfg();
						ChkRangeValid(schRangeSelection);
						SendTskMsg(SCH_AUTORANGE_ID, TSK_INIT, schRangeSelection, SchTskFinish);
						tskState = SCH_MEASURE_DELAY;
						tickOut = 10800000;//3hours
					...
					case SCH_CALIBRATION:
					{
						...
						UpdateSchFlushCfg();
						SendTskMsg(SCH_CALI_ID, TSK_INIT, schRangeSelection, SchTskFinish);
					...
					case SCH_FLUSH:
					{
						...
						UpdateSchFlushCfg();
						assert(MEAS_RANGE_MAX > schRangeSelection);
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, flushSchSteps[schRangeSelection], SchTskFinish);
					...
					case SCH_FLOW_1:
					{
						...
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, triggerFlowStep, SchTskFinish);
			...
			{
				mainActionDetail = 0x0;
				SendTskMsg(SCH_MEAS_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(SCH_CALI_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(SCH_CLEAN_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(SCH_IO_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(FLOW_TSK_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(SCH_AUTORANGE_ID, TSK_FORCE_BREAK, 0, NULL);
			...
			{
				mainActionDetail = 0x0;
				SendTskMsg(SCH_MEAS_ID, TSK_RESETIO, 0, NULL);
				SendTskMsg(SCH_CALI_ID, TSK_RESETIO, 0, NULL);
				SendTskMsg(SCH_CLEAN_ID, TSK_RESETIO, 0, NULL);
				SendTskMsg(SCH_IO_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(FLOW_TSK_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(SCH_AUTORANGE_ID, TSK_FORCE_BREAK, 0, NULL);
				tskState = SCH_FINISH;
			...
			else if ( mainTskState == TSK_FORCE_STOP)
			{
				...
				SendTskMsg(SCH_IO_ID, TSK_FORCE_BREAK, 0, NULL);
				SendTskMsg(FLOW_TSK_ID, TSK_FORCE_BREAK, 0, SchTskFinish);
				SendTskMsg(SCH_AUTORANGE_ID, TSK_FORCE_BREAK, 0, NULL);
				//osDelay(100);
				tskState = SCH_STOP;
			...
			else if ( mainTskState == TSK_INIT)
			{
				...
				else if(tskState != SCH_IDLE)
				{
					...
					{
						TSK_MSG_CONVERT(event.value.p)->callBack(reqResult);
					}
