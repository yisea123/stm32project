...
{
	(void)val;
	SendTskMsg(SCH_AUTORANGE_ID, TSK_SUBSTEP, 0, NULL);
	validResult = val;
}
			...
			else if ( mainTskState == TSK_SUBSTEP)
			{
				...
				switch (tskState)
				{
					...
					case SCH_AR_MEAS:
						...
						SendTskMsg(SCH_MEAS_ID, TSK_INIT, schRangeSelection, ARFinished);
					...
					case SCH_AR_CALI_CHK:
					{
						...
						else
						{
							...
							SendTskMsg(SCH_CALI_ID, TSK_INIT, schRangeSelection, ARFinished);
