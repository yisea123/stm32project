...
typedef struct _T_DATA_OBJ_CLASS
{
    ...
    */
    ...
    //--------------------------------------------------------------------------------------------------
    /*!
     \brief  transform the object or attribute from external into internal format
             this method couldn't return an error-code, because the businessrules has to
             guarantee that no overflow could occur!
     \param  ptrValue object or attribute in internal format
     \param  attributeIndex ==-1 --> Object; >=0 --> Attribute
     \return error-code
    */
    //--------------------------------------------------------------------------------------------------
    uint16_t (*Transform2Internal)(const struct _T_DATA_OBJ *me, void * ptrValue, int16_t attributeIndex );

    //--------------------------------------------------------------------------------------------------
    /*!
     \brief  transform the object or attribute from internal into external format
             this method couldn't return an error-code, because the businessrules has to
             guarantee that no overflow could occur!
     \param  ptrValue object or attribute in internal format
     \param  attributeIndex ==-1 --> object; >=0 --> Attribute
    ...
    */
    //--------------------------------------------------------------------------------------------------
    uint16_t (*Transform2External)(const struct _T_DATA_OBJ *me, void * ptrValue, int16_t attributeIndex );
