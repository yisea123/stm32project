...
__IO uint32_t staDataValve[VALVE_PINS_NUM]					__attribute__ ((section (".configbuf_StaData")));
__IO uint64_t staDataMotorSteps[IDX_MOTOR_MAX] 				__attribute__ ((section (".configbuf_StaData")));
__IO uint32_t lifeSpan_RuningSeconds[LIFESPAN_W_MAX_ID]		__attribute__ ((section (".configbuf_StaData")));
__IO uint32_t lifeSpanInstallTime_ST[LIFESPAN_W_MAX_ID]		__attribute__ ((section (".configbuf_StaData")));
__IO uint32_t lifeSpan_StaticStart[LIFESPAN_W_TUBING_MAX]	__attribute__ ((section (".configbuf_StaData")));
__IO float		fluidVolume_Remain[VOL_MAX_ID]				__attribute__ ((section (".configbuf_StaData")));
__IO PumpValve 	pumpValve[VOL_MAX_ID]						__attribute__ ((section (".configbuf_StaData")));
__IO float		usedVolumePPS[VOL_MAX_ID]					__attribute__ ((section (".configbuf_StaData")));
__IO PowerOnOff powerOnOff_ST								__attribute__ ((section (".configbuf_StaData")));
__IO STA_LifeTime lifeTime_STA 								__attribute__ ((section (".configbuf_StaData")));

static uint32_t fileID2 									__attribute__ ((section (".configbuf_StaData")));

__IO uint32_t lifeSpan_InstalledSeconds[LIFESPAN_W_MAX_ID];

__IO uint32_t lifeSpanTubing_StaticSeconds[LIFESPAN_W_TUBING_MAX] = {0,0,0};
__IO uint32_t communicationStatus[COM_MAX_COUNT] = {0,0,0,0,};
__IO uint32_t communicationHappenST[COM_MAX_COUNT] = {0,0,0,0};
__IO uint32_t	freeRtosTskTick[MAX_TASK_ID] = {0,0,0,0,0,0,};
...
void Sta_UpdateVolumeUsage(const StepConfig* steps)
{
//	OS_Use_Unit(STA_Data.ptrDataSemaphore);
	for(uint16_t i=0;i<VOL_MAX_ID;i++)
	{
		if( (pumpValve[i].valveState == 0xFFFF) || ((pumpValve[i].valveState & steps->valvStatus) == pumpValve[i].valveState))
		{
			float _usedVolume = 0.0f;
			...
			{
				_usedVolume = (float)steps->pump2.steps * usedVolumePPS[i];
				//todo:
			...
			fluidVolume_Remain[i] -= _usedVolume;
			if(fluidVolume_Remain[i] < 0)
				fluidVolume_Remain[i] = 0;
		}

	}
	Trigger_EEPSave((void*)&fluidVolume_Remain[0], sizeof(fluidVolume_Remain),SYNC_CYCLE);
//	OS_Unuse_Unit(STA_Data.ptrDataSemaphore);
}

uint16_t Sta_UpdateStep(const StepConfig* ptrSteps)
{
	uint16_t result = OK;
//	OS_Use_Unit(STA_Data.ptrDataSemaphore);
	if(ptrSteps->pump1.mode == FTP)
		staDataMotorSteps[IDX_PUMP1] += ptrSteps->pump1.steps;
	else
		staDataMotorSteps[IDX_PUMP1] += ptrSteps->pump1.steps/2;
	if(ptrSteps->pump2.mode == FTP)
		staDataMotorSteps[IDX_PUMP2] += ptrSteps->pump2.steps;
	else
		staDataMotorSteps[IDX_PUMP2] += ptrSteps->pump2.steps/2;


	if(ptrSteps->pump3.mode == FTP)
		staDataMotorSteps[IDX_PUMP3] += ptrSteps->pump3.steps;
	else
		staDataMotorSteps[IDX_PUMP3] += ptrSteps->pump3.steps/2;

//
	if(ptrSteps->pump1.pps)
	{
		lifeSpan_RuningSeconds[LIFESPAN_W_TUBING_PUMP1] += (ptrSteps->pump1.steps+ptrSteps->pump1.pps-1)/ptrSteps->pump1.pps;
		lifeSpan_RuningSeconds[LIFESPAN_W_PUMP1] += (ptrSteps->pump1.steps+ptrSteps->pump1.pps-1)/ptrSteps->pump1.pps;
	}
	if(ptrSteps->pump2.pps)
	{
		lifeSpan_RuningSeconds[LIFESPAN_W_TUBING_PUMP2] +=(ptrSteps->pump2.steps+ptrSteps->pump2.pps-1)/ptrSteps->pump2.pps;
		lifeSpan_RuningSeconds[LIFESPAN_W_PUMP2] += (ptrSteps->pump2.steps+ptrSteps->pump2.pps-1)/ptrSteps->pump2.pps;
	}
	if(ptrSteps->pump3.pps)
	{
		lifeSpan_RuningSeconds[LIFESPAN_W_TUBING_PUMP3] += (ptrSteps->pump3.steps+ptrSteps->pump3.pps-1)/ptrSteps->pump3.pps;
		lifeSpan_RuningSeconds[LIFESPAN_W_PUMP3] += (ptrSteps->pump3.steps+ptrSteps->pump3.pps-1)/ptrSteps->pump3.pps;
	}

//	OS_Unuse_Unit(STA_Data.ptrDataSemaphore);
	//todo ->tubing time and reagent/zero/std/..etc
	//stored in staDataFlow
