...
static float GetTemp(uint16_t chn)
{
#define C_A   (9.331840E-4f)   // a0
#define C_B   (2.213881E-4f)   // a1
#define C_C   (1.263128E-7f)   // a3.
	float Rt = GetResistor(chn);
    float Ln_ohm = (float)log(Rt);
    float tempe = 1.0f/(C_A + C_B*Ln_ohm + C_C*Ln_ohm*Ln_ohm*Ln_ohm) - 273.15f;
    if (tempe >= 320.0f)
    	tempe = 320.0f;
    else if (tempe <= -300.67f)
    	tempe = -300.0f;
    else
    {}
...
float GetResistor(uint16_t chn)
{
	static float Rt = 24252.6f;//30C
	float val = ADCValue[6] - (float)ADCValue[chn];
	if(val <= 0.1)
	{
		val = 1.0f;
		TraceMsg(TSK_ID_TEMP_CTRL, "Error in ADC chip for temperature!\n");
	}

	Rt = (10000.0f*(float)ADCValue[chn])/val;//65536/4.096*2.5 == 40000
	return Rt;
}
...
#else
		return (int16_t)(GetTemp(chn)*100.0);
#endif
	}
	else if(chn == 3)
	{
