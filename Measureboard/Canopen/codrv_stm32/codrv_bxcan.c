...
static __IO BOOL_T transmissionIsActive = { CO_FALSE };
...
static __IO UNSIGNED32 * const pCanFilterAddr = CODRV_BXCAN1_ADDR; /*lint !e923 CAN controller address */


/** \var pCan - used CAN controller address */
#  ifdef CONFIG_BXCAN1 
static __IO UNSIGNED32 * const pCan = CODRV_BXCAN1_ADDR; /*lint !e923 CAN controller address */
...
#  endif
#  ifdef CONFIG_BXCAN2
static __IO UNSIGNED32 * const pCan = CODRV_BXCAN2_ADDR; /*lint !e923 CAN controller address */
...
UNSIGNED32 mcr;
UNSIGNED16 i;
__IO UNSIGNED32 *pCanFilter = pCanFilterAddr;
	...
	*/
	...
	if (canEnabled == CO_TRUE) {
		...
		if ((pCan[BXCAN_CANMSR] & BXCAN_CANMSR_INAK) == 0ul) {
			...
			(void)codrvCanStartTransmission(); /* default: -> TX IRQ call at this point */
		}
	}
