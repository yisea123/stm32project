...
*-------------------------------------------------------------------
* $Id: co_odaccess.c 16950 2017-02-24 12:30:22Z boe $
*
*
*-------------------------------------------------------------------
* Changelog:
...
{
UNSIGNED16	left = 0u;
UNSIGNED16	right = (UNSIGNED16)(odAssignCnt - 1u);
...
RET_T		retVal;
	...
	while (left <= right)  {
		/* l + r / 2 */
		middle = (UNSIGNED16)((left + right) >> 1);
		...
		if (pOdAssingTable[middle].index > index)  { /*lint !e960 Note: pointer arithmetic other than array indexing used */
			...
			right = (UNSIGNED16)(middle - 1u);
		} else {
			left = (UNSIGNED16)(middle + 1u);
		}
	...
	right = (UNSIGNED16)((left + (UNSIGNED16)pOdAssingTable[middle].numberOfSubs) - 1u); /*lint !e960 Note: pointer arithmetic other than array indexing used */
	found = CO_FALSE;
...
#endif /* defined(CO_STORE_SUPPORTED) || defined(CO_EVENT_STORE) */
