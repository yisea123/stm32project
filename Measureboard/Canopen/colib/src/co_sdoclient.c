...
*-------------------------------------------------------------------
* $Id: co_sdoclient.c 19915 2017-09-26 13:13:54Z boe $
*
*
*-------------------------------------------------------------------
*
...
UNSIGNED16	index;
	...
	index = (UNSIGNED16)(index | pRecData->data[1]);
...
UNSIGNED16	index;
	...
	/* check for correct index/subindex */
	index = (UNSIGNED16)((UNSIGNED16)pRecData->data[2] << 8);
	index = (UNSIGNED16)(index | pRecData->data[1]);
	if ((pSdo->index != index) || (pSdo->subIndex != pRecData->data[3]))  {
		/* send abort */
		icoSdoClientAbort(pSdo, RET_SDO_INVALID_VALUE);
...
UNSIGNED16	idx;
	...
	for (cnt = 0u; cnt < sdoClientCnt; cnt++)  {
		...
		sdoClient[idx].trCobId = CO_COB_INVALID;
		sdoClient[idx].recCobId = CO_COB_INVALID;
		(void)coOdGetDefaultVal_u8((UNSIGNED16)(0x1280u + cnt), 3u, &sdoClient[idx].node);
	}
}
