...
*-------------------------------------------------------------------
* $Id: gateway_emtas.c 11215 2015-10-27 13:27:34Z ro $
*
*
*-------------------------------------------------------------------
*
...
extern uint8_t CDC_Transmit_HS(
		uint8_t* Buf,
		uint16_t Len
	);
...
static UNSIGNED8 usbRxBuffer[USB_RX_DATA_SIZE];
UNSIGNED32 usbRxSize;

static UNSIGNED8 * pIrqBuf;
static UNSIGNED32 irqSize;
#endif
...
printf("\n");
		...
		if (CDC_Transmit_HS((uint8_t*) str, size + 1) == USBD_BUSY) {
			usbTxFlag = 1u;
		} else {
			usbTxFlag = 0u;
		}
...
static UNSIGNED32 maxLen = 0ul; // for debugging (max CDC_DATA_HS_IN_PACKET_SIZE)
...
UNSIGNED8 fCanMsg = 0u;
	...
	// TX polling
	if (usbTxFlag == 1) {
		if (CDC_Transmit_HS(NULL, 0) == USBD_OK) {
			usbTxFlag = 0;
		}
	}
}
