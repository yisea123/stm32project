...
uint32_t FLASH_SIZE=32*1024*1024;	//FLASH 32M
...
uint16_t W25QXX_ReadID(void)
{
	uint16_t Temp = 0;
	//发送读取ID命令
	uint8_t sendBuf[6] = {0x90,0x00,0x00,0x00,0xFF,0xFF};
	uint8_t rxdata[6];
	W25QXX_CS=0;
	HAL_SPI_TransmitReceive(&hspi5,&sendBuf[0],&rxdata[0],6, 1000);
	W25QXX_CS=1;
	Temp = (uint16_t)((uint16_t)(rxdata[4]<<8) + rxdata[5]);
	return Temp;
}

static void SPI_WriteCmdAdr(uint8_t cmd, uint32_t adr)
{
	uint8_t sendData[5];
	if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
	{
		sendData[0] = cmd;
		sendData[1] = (uint8_t)((adr)>>24);
		sendData[2] = (uint8_t)((adr)>>16);
		sendData[3] = (uint8_t)((adr)>>8);
		sendData[4] = (uint8_t)adr;
		HAL_SPI_Transmit(&hspi5,sendData,5,1000);
	}
	else
	{
		sendData[0] = cmd;
		sendData[1] = (uint8_t)((adr)>>16);
		sendData[2] = (uint8_t)((adr)>>8);
		sendData[3] = (uint8_t)adr;
		HAL_SPI_Transmit(&hspi5,sendData,4,1000);
	}
}
//读取SPI FLASH
//在指定地址开始读取指定长度的数据
...
{
	W25QXX_CS=0;                            //使能器件
    SPI_WriteCmdAdr(W25X_ReadData, ReadAddr);
...
void W25QXX_Write_Page(uint8_t* pBuffer,uint32_t WriteAddr,uint16_t NumByteToWrite)
{
	...
	W25QXX_CS=0;                            //使能器件
	SPI_WriteCmdAdr(W25X_PageProgram, WriteAddr);
