					 ...
					 OS_RSEMA * ptrDataSemaphore)
{
	...
	else
	{
		...
		{
			// transform to internal view if defined
			if(myself.pClass->Transform2Internal)
			{
				myself.pClass->Transform2Internal(me,ptrValue,attributeIndex);
			}
			...
			// transform to external view if defined
			if(myself.pClass->Transform2External)
			{
				myself.pClass->Transform2External(me,ptrValue,attributeIndex);
			}
		}
	}
