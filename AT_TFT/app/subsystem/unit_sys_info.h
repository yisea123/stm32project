...
extern const uint8_t 	Type_Info[INFO_LEN];
extern const uint16_t 	DeviceID;
extern const uint8_t 	DeviceType[INFO_LEN];
extern const uint32_t   VersionCode;
...
uint16_t Put_SysInfo(const T_UNIT *me, uint16_t objectIndex, int16_t attributeIndex,
					void * ptrValue);
#define Sys_Get(objectIndex, attributeIndex, ptrValue)		 Get_SysInfo(&sysInfo, objectIndex, attributeIndex, ptrValue)
#ifdef __cplusplus
 }
#endif
