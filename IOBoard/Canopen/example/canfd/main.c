...
static void sdoClientReadInd(UNSIGNED8 sdoNr, UNSIGNED16 index,
		UNSIGNED8 subIndex, UNSIGNED32 len, UNSIGNED32 result);
...
RET_T		retVal;
	...
	if (node == 1)  {
	    	/* SDO client part */
	    	/* initialize the object dict entries */
		...
		/* setup sdo connection to node 127 */
		coOdSetCobid(0x1280, 1, 0x600 + 127);
		coOdSetCobid(0x1280, 2, 0x580 + 127);

		if (testmode == 0) {
			startSdoTransfer(0);
		} else {
			/* domain transfer example
			 * "dsize" is used domain size */
			/* time measurement */
			timeMeasurement(START);
			retVal = coSdoWrite(1, 0x2020, 0, pDomainBuf, dsize, 0, 1000);
	...
	const char* gedicht = "Wenn der Tag erwacht und die Sonne lacht, ziehen wir Sachen an und gehen dann";
	memcpy(strg50, gedicht, 50);
...
RET_T		retVal;
	...
	if (idx < (sizeof(sdoTransmitTable) / sizeof(SDO_TRANSMIT_TABLE)))  {
		...
		if (sdoTransmitTable[idx].write != CO_TRUE)  {
			retVal = coSdoRead(1, sdoTransmitTable[idx].index, 0,
				sdoTransmitTable[idx].pBuf, sdoTransmitTable[idx].size, 0, 1000);
		} else {
			retVal = coSdoWrite(1, sdoTransmitTable[idx].index, 0,
				sdoTransmitTable[idx].pBuf, sdoTransmitTable[idx].size, 0, 1000);
		}
...
/*********************************************************************/
void sdoClientWriteInd(UNSIGNED8 sdoNr, UNSIGNED16 index, UNSIGNED8 subIndex,
		UNSIGNED32 result)
{
	printf("sdo write to %x:0 returns %lx\n", index, result);
	...
	if (testmode == 0) {
	    /* start next transfer defined in sdoTransmitTable */
	    coTimerStart(&sdoTimer, 2000000, startSdoTransfer, NULL, CO_TIMER_ATTR_ROUNDUP);
	}
}


void sdoClientReadInd(UNSIGNED8 sdoNr, UNSIGNED16 index, UNSIGNED8 subIndex,
		UNSIGNED32 len, UNSIGNED32 result)
{
	printf("sdo read from %x:0 returns %lx\n", index, result);
