...
//						exMsg.callBack = TskCallBackLoc;
					...
					else if(IN_RANGE(subStepId,SPECIAL_IDX_0, SPECIAL_IDX_MAX))
					{
						...
						case PREHEATING:
							...
							heatMsg.detail.val = GetPreHeatTemp();
							SendTskMsg(TEMPERATURE_ID,TSK_INIT,heatMsg.heatMsg_Val, NULL);
						...
						case HEATING_STOP:
							tskState = SUBSTEP_HEAT;
							heatMsg.detail.mode = HEAT_MODE_STOP;			//
							heatMsg.detail.val = 0;
							SendTskMsg(TEMPERATURE_ID,TSK_INIT, heatMsg.heatMsg_Val, NULL);
							break;
						case PUMP_RESET:
							SendTskMsg(FLOW_MIX_CTL_ID,TSK_RESETIO, 0, NULL);
...
//					blankMeasure = 0;
				...
				case SUBSTEP_FINISH:
					...
					{
						localMsg.callBackFinish(taskID,0);
					}
					//MsgPush (FLOW_SUBSTEP_ID, (uint32_t)&localMsg, 0);
					break;
				default:
