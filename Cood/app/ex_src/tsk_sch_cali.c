		...
		else
		{
			...
			if( mainTskState == TSK_SUBSTEP)
			{
				...
				switch(tskState)
				{
					...
					case SCH_CALI_STD0_PRE:
						...
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, caliPreActStd[schCaliRangeSel][0], CaliFinished, NULL);
						break;
					case SCH_CALI_STD0_PRE_DELAY:
						tskState = SCH_CALI_STD0;
						MsgPush (SCH_CALI_ID, (uint32_t)&locMsg, 0);
					...
					case SCH_CALI_STD1_PRE:
						...
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, caliPreActStd[schCaliRangeSel][1], CaliFinished, NULL);
						break;
					case SCH_CALI_STD1_PRE_DELAY:
						tskState = SCH_CALI_STD1;
						MsgPush (SCH_CALI_ID, (uint32_t)&locMsg, 0);
					...
					case SCH_CALI_STD0:
					{
						...
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, caliSchSteps[schCaliRangeSel][0], CaliFinished, NULL);
					...
					case SCH_CALI_STD1:
					{
						...
						assert(MEAS_RANGE_MAX > schCaliRangeSel);
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, caliSchSteps[schCaliRangeSel][1], CaliFinished, NULL);
					...
					case SCH_CALI_FINISH:
						...
						{
							locMsg.callBackFinish(validResult,0);
						}
						tskState = SCH_CALI_IDLE;
						break;
					case SCH_CALI_IDLE:
			...
			else if ( mainTskState == TSK_INIT)
			{
				...
				else
				{
					...
					{
						TSK_MSG_CONVERT(event.value.p)->callBackFinish(DEVICE_BUSY_STATE);
					}
