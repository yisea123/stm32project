...
{
	(void)val;
	SendTskMsg(SCH_MEAS_ID, TSK_SUBSTEP, 0, NULL, NULL);
}
...
void CheckMeasRange(uint16_t rangeIdx, float measValue)
{
	...
	if(val1)
		SendTskMsg(SCH_IO_ID, TSK_INIT, RO_ALARMLOW_SET , NULL, NULL);
	else
		SendTskMsg(SCH_IO_ID, TSK_INIT, RO_ALARMLOW_CLR, NULL, NULL);

	val1 =  ( measValue > measRangeAlarmLimit[1])? 2: 0;
	if(val1)
		SendTskMsg(SCH_IO_ID, TSK_INIT, RO_ALARMHIGH_SET, NULL, NULL);
	else
		SendTskMsg(SCH_IO_ID, TSK_INIT, RO_ALARMHIGH_CLR , NULL, NULL);
			...
			switch(tskState)
			{
				case SCH_MEAS_SAMPLEFLOW_DELAY:
				{
					...
					if(sampleFlow != OK)
					{
						...
						SendTskMsg(SCH_IO_ID, TSK_INIT, RO_PRETREATMENT_CLR, NULL, NULL);
				...
				switch(tskState)
				{
					case SCH_MEAS_PRETREATMENT:
						...
						if(measSch.preTreatDelay)
						{
							...
							flowStepRun[0].startTime = GetCurrentST();
							flowStepRun[0].duringTime = measSch.preTreatDelay;
							SendTskMsg(SCH_IO_ID, TSK_INIT, RO_PRETREATMENT_SET, NULL, NULL);
						}
					...
					case SCH_MEAS_MEASURE:
					{
						...
						SendTskMsg(FLOW_TSK_ID, TSK_INIT, measureSchSteps, MeasureFinished, NULL);
					...
					case SCH_MEAS_FINISH:
						...
						if(measureType == MSK_MEAS_STD1)
						{
						//	HandlePostVerify(schMeasSel, measResultRealTime.measValue, measVal_Final);
						}
						...
						{
							locMsg.callBackFinish(measResult,0);
						}
						break;
					case SCH_MEAS_IDLE:
						break;
...
#define POLL_CMD
				...
				else
				{
					...
					{
						TSK_MSG_CONVERT(event.value.p)->callBackFinish(DEVICE_BUSY_STATE, 0);
					}
