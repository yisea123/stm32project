...
#define TOTAL_COUNT_AD				32
#define TIMES_PER_SECOND			2
#define DAT_NUM_EACH				8
#define MAX_AD_SIZE					(TIMES_PER_SECOND*DAT_NUM_EACH)//actual is 128
...
int32_t led0Pos = 0x1FFFF;
int32_t led1Pos = 0x1FFFF;

extern volatile ENUM_MEA_STEP mea_step;
...
static void GetADC_Data1(MEAS_DATA* _msgSt, int32_t idx)
{
	...
	static 	int32_t countCycle = 0;
	int countTime = 0;
	int measStepLoc = mea_step;
	static int32_t countTotal = 0;
	if(mea_step != MEA_STEP_START)
		measStepLoc = 1;
	...
	adcIdx = (countCycle % TIMES_PER_SECOND) * DAT_NUM_EACH + measStepLoc*4 + idx;
	...
	if(ret1 == OK)
	{
		...
		{
			countTotal++;
			if( measStepLoc != MEA_STEP_START)
			{
				led0Pos = (_msgSt->ref[adcIdx] - _msgSt->ref[adcIdx-1] - _msgSt->ref[adcIdx-2] + _msgSt->ref[adcIdx-3]);
				led1Pos = (_msgSt->meas[adcIdx] - _msgSt->meas[adcIdx-1] - _msgSt->meas[adcIdx-2] + _msgSt->meas[adcIdx-3]);
			}
			else
			{
				countCycle++;
			}
			if(countTotal >= TOTAL_COUNT_AD )
			{
		...
		if(evt.status==osEventSignal)
		{
			...
			switch (evt.value.signals)
			{
				...
				case AD_SAMPLING_CHNL_SHORT:
					...
					sortLong = AD_SAMPLING_CHNL_SHORT;
					break;
				case AD_SAMPLING_CHNL_LONG:
					DevAD_SelectChn(AD7172_MEA_ID, AD_CHN_LONG);
					DevAD_SelectChn(AD7172_REF_ID, AD_CHN_LONG);
					sortLong = AD_SAMPLING_CHNL_LONG;
					break;
