...
static volatile E_AD_TIME_STATE ad_test_state = e_ad_t12;
volatile ENUM_MEA_STEP mea_step = MEA_STEP_START;
...
static const PinInst MeaGainPins[MEA_DATA_MAX][REF_GIN_PIN_NUM]=
{

	{
		...
		{GAIN_L_MEA3_GPIO_Port,	GAIN_L_MEA3_Pin}, //pk5
	...
	//short
	{
			...
			{GAIN_S_MEA3_GPIO_Port,	GAIN_S_MEA3_Pin}, //pj15
...
static const PinInst RefGainPins[MEA_DATA_MAX][REF_GIN_PIN_NUM]=
{

			{
				...
				{GAIN_L_REF3_GPIO_Port,	GAIN_L_REF3_Pin},//pd7
				{LED_880_L_IC_GPIO_Port,LED_880_L_IC_Pin},//??
			},
#if 0
			{
			...
			//short
			{
					...
					{GAIN_S_REF3_GPIO_Port,	GAIN_S_REF3_Pin},//pd1
					{LED_660_S_IC_GPIO_Port,LED_660_S_IC_Pin},//??
			},
#if 0
			{
...
static void LampOnOff(ENUM_MEA_STEP step, int on_off)
{
	...
	{
		if (on_off)
			OutPinOne(&LampPin[MEA_STEP_660nm_SHORT],GPIO_PIN_RESET);
		else
			OutPinOne(&LampPin[MEA_STEP_660nm_SHORT],GPIO_PIN_SET);
...
static void SetADGain(ENUM_MEA_STEP step)
{
	if(MEA_STEP_START == step)
	{
		OutPin4(&MeaGainPins[step][0],	4, gainMaskMeaSet[step]);
		OutPin4(&RefGainPins[step][0],	4, gainMaskRefSet[step]);
	}
	else
	{
		OutPin4(&MeaGainPins[MEA_STEP_660nm_SHORT][0],	4, gainMaskMeaSet[MEA_STEP_660nm_SHORT]);
		OutPin4(&RefGainPins[MEA_STEP_660nm_SHORT][0],	4, gainMaskRefSet[MEA_STEP_660nm_SHORT]);
