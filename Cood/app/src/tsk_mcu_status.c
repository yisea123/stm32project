...
void StartMCUStatusTask(void const * argument)
{
	...
	while (TASK_LOOP_ST)
	{
		...
		if (event.status != osEventMessage)
		{
			...
			uint32_t val = GetADC3_Value(ADC3_12V_SUPPLY);
			AdcVolts[ADC3_12V_SUPPLY] = (uint16_t)(val * 4.833984375);//???? //todo

			//
			val = GetADC3_Value(ADC3_CURR_MONITOR);
			AdcVolts[ADC3_CURR_MONITOR] = (uint16_t)(val * 2.44140625);//???? //todo
		...
		else
		{
			...
			else if ( mainTskState == TSK_INIT)
			{
				...
				SendTskMsgLOC(MCU_STATUS_ID, &localMsg);
			}
