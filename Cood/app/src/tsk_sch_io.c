...
void StartIOTask(void const * argument)
{
	...
	localMsg.val.value = IO_POLL_INIT;
	TracePrint(taskID,"task started  \n");
	tskState = IO_POLL_INIT;
	SendTskMsgLOC(SCH_IO_ID, &localMsg);//start by it self
	while (TASK_LOOP_ST)
	{
		...
		if (event.status != osEventMessage)
		{
			...
			case RO_POLL_CLR_TIMEOUT:
				...
				RO_Output(tskState);
				tskState = IO_FINISH;
				SendTskMsgLOC(SCH_IO_ID, &localMsg);
				break;
		...
		else
		{
			...
			else if ( mainTskState == TSK_FORCE_BREAK)
			{
				...
				MsgPush(SCH_IO_ID, (uint32_t ) &localMsg, 0);
			...
			else if ( mainTskState == TSK_INIT)
			{
				...
				localMsg = *(TSK_MSG_CONVERT(event.value.p));
				localMsg.tskState = TSK_SUBSTEP;
				if(localMsg.val.value != 0)
				{
					IO_State tskState1 = (IO_State)localMsg.val.value;
