...
volatile uint16_t		trigNum[2] = {0,0};
...
uint16_t CalcOffset(float* absVal0, uint16_t rangeIdx)
{
	...
	float OffsetLong = absVal0[MEA_STEP_660nm_LONG];

	float OffsetShort = absVal0[MEA_STEP_660nm_SHORT];
...
//long are only allpied for range idx 0 and 1;
	...
	float slopeLong = concentration / ( (absVal1[MEA_STEP_660nm_LONG]) - OffsetLong );

//short shall be always valid for all range idx;
	float Offset = absVal0[MEA_STEP_660nm_SHORT];
	float slope = concentration / ( (absVal1[MEA_STEP_660nm_SHORT]) - Offset );
	...
	//		UpdateSchFlushCfg();
		...
		case OBJ_SYSTEM_BREAK:
		{
			if (systemBreakState != 0)
			{
				...
				ClearTriggers();
				SendTskMsg(SCH_ID, TSK_FORCE_BREAK, SCH_TSK_IDLE, NULL, NULL);
		...
		case OBJ_SYSTEM_STOP:
		{
			...
			else
			{
				...
				SCH_Put(OBJ_SCH_MODE, WHOLE_OBJECT,(void*)&mode);
				ClearTriggers();			
				SendTskMsg(SCH_ID, TSK_FORCE_STOP, SCH_TSK_IDLE, NULL, NULL);
