...
CAN_SIMU_T canSimu[] = {
	{ 0x67ful,	8u, { 0x40u, 0u, 0x10u, 0u, 0u, 0u, 0u, 0u }}, /* SDO read 0x1000 */
	{ 0x0ul,	2u, { 1u, 0u, 0u, 0u, 0u, 0u, 0u, 0u }}, /* NMT Operational */
	{ 0x67ful,	8u, { 0x23u, 4u, 0x18u, 1u, 0x23u, 1u, 0u, 0u}}, /* SDO Write 0x1804:1 */
	{ 0x80ul,	0u, { 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u }}, /* Sync */
};
static UNSIGNED8	simData = 0u;

	if (simData >= (sizeof(canSimu) / sizeof(CAN_SIMU_T))) {
		return;
	}
/* --- Simulation End ---- */
...
/* --- Simulation ---- */
	...
	pRecBuf->canCob.canId = canSimu[simData].id;
	pRecBuf->len = canSimu[simData].len;
	for (j = 0u; j < canSimu[simData].len; j++) {
		pRecBuf->data[j] = canSimu[simData].data[j];
	}
...
/* --- Timer Simulation !!! Normal not part of the CAN driver !!! ---- */
	{
		extern void codrvTimerISR(void);
		codrvTimerISR();
	}
/* --- Timer Simulation !!! Normal not part of the CAN driver !!! ---- */
#endif
