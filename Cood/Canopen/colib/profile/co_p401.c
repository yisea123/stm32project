...
*-------------------------------------------------------------------
* $Id: co_p401.c 20152 2017-10-30 13:06:37Z boe $
*
*
*-------------------------------------------------------------------
*
...
RET_T retval;
	...
	if (pDigOut != NULL) {
		...
		(void)coOdGetObj_u8(0x6200u + CO_401_PROFILE_OFFSET, (UNSIGNED8)port,
				&value); 
		/* get the polarity invert value for this port */
		retval = coOdGetObj_u8(0x6202u + CO_401_PROFILE_OFFSET, (UNSIGNED8)port,
				&invert); 
		if (retval == RET_OK) {
		...
		/* Call the HE API */
		pDigOut((UNSIGNED8)port, (UNSIGNED8)value);
	}
}
...
INTEGER16 value;
	...
	if (pAnOut != NULL) {
		 ...
		 * All parameters are coming from the stack,
		 * no need to check the return value */
		(void)coOdGetObj_i16(0x6411u + CO_401_PROFILE_OFFSET, (UNSIGNED8)port, &value);
		...
		for (port = 1u; port <= maxport; port++) {
			...
			if (retval == RET_OK) {
				...
				if (errorMode == 1u) {
					...
					/* Call the HE API */
					pAnOut(port, (INTEGER16)errorValue);
				}
		...
		if ((retval == RET_OK) && (maxMappings > 0u)) {
			...
			for (subIndex = 1u; subIndex <= maxMappings; subIndex++) {
				...
				if (retval == RET_OK) {
					...
					/* add pdo */
					mappingTable[count] = (UNSIGNED32)(mappingTable[count] | (UNSIGNED32)(pdo & 0xffu));
				}
