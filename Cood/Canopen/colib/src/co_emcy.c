...
*-------------------------------------------------------------------
* $Id: co_emcy.c 16941 2017-02-24 10:33:09Z boe $
*
*
*-------------------------------------------------------------------
* Changelog:
...
# ifdef CO_EVENT_EMCY_CONSUMER_CNT
	...
	while (cnt > 0u)  {
		...
		emcyConsTable[cnt]((UNSIGNED8)(pRecData->msg.canCob.canId & 0x7fu),
				(UNSIGNED16)((UNSIGNED16)pRecData->msg.data[0] \
...
UNSIGNED16	idx;
	...
	for (i = 0u; i < emcyConsumerCnt; i++)  {
		idx = i;
		retVal = coOdGetDefaultVal_u32(0x1028u, (UNSIGNED8)(i + 1u), &emcyCons[idx].cobId);
		if (retVal != RET_OK)  {
