...
*-------------------------------------------------------------------
* $Id: co_pdo.c 20173 2017-10-30 14:14:37Z boe $
*
*
*-------------------------------------------------------------------
*
...
# endif /* CO_EVENT_PDO_UPDATE_CNT */
	...
	for (cnt = 0u; cnt < pPdo->mapTableConst->mapCnt; cnt++)  {
		...
		offs = (UNSIGNED8)(offs + pPdo->mapTableConst->mapEntry[cnt].len);
	}
...
BOOL_T		changed;
	...
	for (cnt = 0u; cnt < pRecPdo->mapTableConst->mapCnt; cnt++)  {
		...
		offs = (UNSIGNED8)(offs + pRecPdo->mapTableConst->mapEntry[cnt].len);
	}
    ...
    )
{
	...
	/* set new indication function as first at the list */
	pdoEventUpdateTable[pdoEventTableCnt] = pFunction;
...
PDO_TR_MAP_TABLE_T map;
	...
	if (mapCnt > icoOdGetNumberOfSubs((UNSIGNED16)(0x1A00u + (pPdo->pdoNr - 1u))))  {
		return(255u);
	}
...
# endif /* CO_TR_PDO_DYN_MAP_ENTRIES */
		...
		offs = (UNSIGNED8)(offs + pMap->mapEntry[cnt].len);
	}
...
PDO_TR_MAP_TABLE_T *pMap;
	...
	numberOfSubs = (UNSIGNED8)(icoOdGetNumberOfSubs(index) - 1u);

	/* for all mapping entries */
	for (cnt = 0u; cnt < numberOfSubs; cnt++)  {
		retVal = coOdGetDefaultVal_u32(index, (UNSIGNED8)(cnt + 1u), &mapEntry);
		if (retVal != RET_OK)  {
			return(retVal);
		}
...
PDO_REC_MAP_TABLE_T map;
	...
	/* mapCnt > max sub index */
	if (mapCnt > icoOdGetNumberOfSubs((UNSIGNED16)(0x1600u + (pPdo->pdoNr - 1u))))  {
		return(255u);
	}
	offs = 0u;
	for (cnt = 0u; cnt < mapCnt; cnt++)  {
...
UNSIGNED16	pdoIdx;
	...
	for (cnt = 0u; cnt < pdoTrCnt; cnt++)  {
		...
		retVal = coOdGetDefaultVal_u32((UNSIGNED16)(0x1800u + pTrPdo->pdoNr - 1u), 1u, &u32);
		if (retVal == RET_OK)  {
		...
		if ((pTrPdo->pdoNr >= 1u) && (pTrPdo->pdoNr <= 4u))  {
			...
			pdoIdx = (UNSIGNED16)(pTrPdo->pdoNr - 1u);
			pTrPdo->cobId = u32
		...
		/* transmission type */
		retVal = coOdGetDefaultVal_u8((UNSIGNED16)((0x1800u + pTrPdo->pdoNr) - 1u), 2u, &u8);
		if (retVal == RET_OK)  {
			pTrPdo->transType = u8;
		}
		retVal = coOdGetDefaultVal_u16((UNSIGNED16)((0x1800u + pTrPdo->pdoNr) - 1u), 3u, &u16);
		if (retVal == RET_OK)  {
			pTrPdo->inhibit = u16;
		}
		retVal = coOdGetDefaultVal_u16((UNSIGNED16)((0x1800u + pTrPdo->pdoNr) - 1u), 5u, &u16);
		if (retVal == RET_OK)  {
			pTrPdo->eventTime = u16;
		}
		retVal = coOdGetDefaultVal_u8((UNSIGNED16)((0x1800u + pTrPdo->pdoNr) - 1u), 6u, &u8);
		if (retVal == RET_OK)  {
...
# ifdef CO_TR_PDO_DYN_MAP_ENTRIES
		if (pTrPdo->dynMapping == CO_TRUE)  {
			(void)setupPdoTrMapTable(pTrPdo->mapTableIdx,
				(UNSIGNED16)((0x1a00u + pTrPdo->pdoNr) - 1u));
		}
# endif /* CO_TR_PDO_DYN_MAP_ENTRIES */
	}
#endif /* CO_PDO_TRANSMIT_CNT */
	...
	for (cnt = 0u; cnt < pdoRecCnt; cnt++)  {
		...
		/* get disable and RTR bit from OD */
		retVal = coOdGetDefaultVal_u32((UNSIGNED16)(0x1400u + pRecPdo->pdoNr - 1u), 1u, &u32);
		if (retVal == RET_OK)  {
			u32 &= (CO_COB_INVALID | CO_COB_RTR_BIT);
		} else {
			u32 = 0u;
		...
		if ((pRecPdo->pdoNr >= 1u)  && (pRecPdo->pdoNr <= 4u))  {
			/* calculate cobs for predefined connection set */
			pdoIdx = (UNSIGNED16)(pRecPdo->pdoNr - 1u);
			pRecPdo->cobId = u32
		...
		/* transmission type */
		retVal = coOdGetDefaultVal_u8((UNSIGNED16)(0x1400u + pRecPdo->pdoNr - 1u), 2u, &u8);
		if (retVal == RET_OK)  {
			pRecPdo->transType = u8;
		}
		retVal = coOdGetDefaultVal_u16((UNSIGNED16)(0x1400u + pRecPdo->pdoNr - 1u), 3u, &u16);
		if (retVal == RET_OK)  {
			pRecPdo->inhibit = u16;
		}
		retVal = coOdGetDefaultVal_u16((UNSIGNED16)(0x1400u + pRecPdo->pdoNr - 1u), 5u, &u16);
		if (retVal == RET_OK)  {
...
# ifdef CO_REC_PDO_DYN_MAP_ENTRIES
		if (pRecPdo->dynMapping == CO_TRUE)  {
			(void)setupPdoRecMapTable(pRecPdo->mapTableIdx,
					(UNSIGNED16)(0x1600u + pRecPdo->pdoNr - 1u));
		}
# endif /* CO_REC_PDO_DYN_MAP_ENTRIES */
...
# ifdef CO_TR_PDO_DYN_MAP_ENTRIES
		/* dynamic mapping */
		mapTableIdx = createPdoTrMapTable((UNSIGNED16)((0x1a00u + pdoNr) - 1u));
...
# ifdef CO_REC_PDO_DYN_MAP_ENTRIES
		/* dynamic mapping */
		mapTableIdx = createPdoRecMapTable((UNSIGNED16)((0x1600u + pdoNr) - 1u));
		if (mapTableIdx == 0xffffu)  {
			return(RET_MAP_LEN_ERROR);
		}
