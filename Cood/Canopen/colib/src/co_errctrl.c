...
*-------------------------------------------------------------------
* $Id: co_errctrl.c 19917 2017-09-26 13:14:54Z boe $
*
*
*-------------------------------------------------------------------
* Changelog:
...
typedef struct {
	CO_TIMER_T			timer;
#ifdef CO_EMTAS_INTERNAL
#else /* CO_EMTAS_INTERNAL */
	COB_REFERENZ_T		cob;
#endif /* CO_EMTAS_INTERNAL */
	UNSIGNED16			hbTime;
	UNSIGNED8			node;
	UNSIGNED32			odVal;
...
# ifdef CO_HB_CONSUMER_CNT
	for ( i = 0u; i < CO_HB_CONSUMER_CNT; i++)  {
		if (hbConsumer[i].node == (pRecData->msg.canCob.canId & 0x7fu))  {
			pHbC = &hbConsumer[i];
			break;
		}
	}
...
#ifdef CO_HB_CONSUMER_CNT
		case 0x1016u:
			retVal = setupHbConsumer( (UNSIGNED8)(subIndex - 1u));
			break;
#endif /* CO_HB_CONSUMER_CNT */
...
#endif /* CO_EMTAS_INTERNAL */
			retVal = coOdGetDefaultVal_u32(0x1016u, (UNSIGNED8)(i + 1u), &u32);
			if (retVal == RET_OK)  {
...
CO_HB_CONSUMER_T		*pHbCons;
	...
	/* setup monitoring */
#ifdef CO_EMTAS_INTERNAL
	icoCobSetIgnore(hbConsCobRef, 0x7ful);
