...
uint16_t LB_Layer7_Decode(MsgFrame* ptrMsgFrame)
{
	...
	switch(func)
	{
		case CMD_POLL:
			...
			break;
		case CMD_READ_OBJ:
			ret = LB_APP_ReadObj(ptrMsgFrame);
			break;
		case CMD_WRITE_OBJ:
		...
		case CMD_READ_TYPE:
			ret = LB_APP_ReadType(ptrMsgFrame);
			break;
		case CMD_READ_LENGTH:
			ret = LB_APP_ReadLength(ptrMsgFrame);
			break;
		case CMD_READ_ATR_NUM:
			ret = LB_APP_ReadAtrNum(ptrMsgFrame);
			break;
		case CMD_READ_RANGE:
			//todo
			...
			break;
		case CMD_READ_NAME:
			ret = LB_APP_ReadName(ptrMsgFrame);
			break;
		case CMD_READ_ADRINFO:
			ret = LB_APP_ReadAdrInfo(ptrMsgFrame);
			break;
		case CMD_READ_MEM:
