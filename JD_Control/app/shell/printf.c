...
#define PRINT_USART_INIT	MX_USART2_UART_Init
...
static SemaphoreHandle_t lockPrint_user = NULL;
...
void Init_PrintSem()
{
	...
	if(lockPrint_user == NULL)	lockPrint_user = OS_CreateSemaphore();
...
void PrintChn_user(uint8_t* msgTable,uint8_t enabled, uint16_t chn, const char* str, ...)
{
	static uint8_t lcBuff[MAX_PRINT_SIZE+8];
	uint16_t chnPrint = CheckMsgPrintEnable(msgTable,chn,enabled);
	PrintSt* buffStRet = NULL;
